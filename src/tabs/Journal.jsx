...
const STORAGE_KEY = 'portfolio_v2_journal';
...
export default function Journal() {
  ...
  const startEdit = (e) => {
    setForm({ week: e.week, notes: e.notes, bought: e.bought, trimmed: e.trimmed || '', confidence: e.confidence, rating: e.rating });
    setEditing(e);
    setView('edit');
  };
    ...
    return (
      <div style={{ padding: '16px 16px calc(100px + var(--safe-bottom))', overflowY: 'auto', height: '100%' }}>
        ...
        {[
          ...
          { label: 'WHAT DID YOU BUY?', key: 'bought', rows: 2, placeholder: 'Tickers, amounts, which week of rotation…' },
          { label: 'DID YOU TRIM ANYTHING?', key: 'trimmed', rows: 2, placeholder: 'Ticker, shares trimmed, gain realized…' },
          { label: 'CONFIDENCE NOTE', key: 'confidence', rows: 2, placeholder: 'How confident in current holdings?' },
        ].map(f => (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 8 }}>
            {PROMPTS.map((p, i) => (
              <button key={i} onClick={() => setForm(f => ({ ...f, notes: f.notes + (f.notes ? '\n\n' : '') + p + '\n' }))}
                style={{ fontSize: 8, color: 'var(--text3)', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 4, padding: '3px 7px', cursor: 'pointer', fontFamily: 'var(--mono)' }}>
                + {p.slice(0, 28)}…
              </button>
            ))}
          </div>
  ...
  return (
    <div style={{ padding: '16px 16px calc(100px + var(--safe-bottom))', overflowY: 'auto', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 9, color: 'var(--text3)', letterSpacing: '0.2em', marginBottom: 2 }}>WEEKLY LOG</div>
      ...
      {entries.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text4)' }}>
          <div style={{ fontSize: 28, fontFamily: 'var(--display)', marginBottom: 8, color: 'var(--bg3)' }}>NO ENTRIES YET</div>
          <div style={{ fontSize: 11 }}>Start your weekly log to track your investment journey</div>
        </div>
      )}
        ...
        <div key={e.id} style={entryCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            <div>
              ...
              <div style={{ fontSize: 9, color: 'var(--text3)' }}>{new Date(e.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
            </div>
          ...
          {e.bought && (
            <div style={{ marginBottom: 8, padding: '7px 10px', background: 'var(--bg3)', borderRadius: 5, borderLeft: '2px solid #00e5ff44' }}>
              <div style={{ fontSize: 7, color: 'var(--text3)', marginBottom: 2 }}>BOUGHT</div>
              <div style={{ fontSize: 11, color: 'var(--text2)' }}>{e.bought}</div>
            </div>
          )}
          {e.trimmed && (
            <div style={{ marginBottom: 8, padding: '7px 10px', background: 'var(--bg3)', borderRadius: 5, borderLeft: '2px solid #69ff4744' }}>
              <div style={{ fontSize: 7, color: 'var(--text3)', marginBottom: 2 }}>TRIMMED</div>
              <div style={{ fontSize: 11, color: 'var(--text2)' }}>{e.trimmed}</div>
            </div>
          )}
...
const saveBtn   = { width: '100%', padding: 14, background: 'var(--cyan)', border: 'none', borderRadius: 10, color: 'var(--bg)', fontSize: 12, letterSpacing: '0.12em', fontFamily: 'var(--mono)', fontWeight: 500, cursor: 'pointer' };
const newBtn    = { padding: '8px 16px', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 8, color: 'var(--cyan)', fontSize: 10, letterSpacing: '0.1em', fontFamily: 'var(--mono)', cursor: 'pointer' };
...
const editBtn   = { padding: '6px 12px', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 6, color: 'var(--text2)', fontSize: 9, letterSpacing: '0.1em', fontFamily: 'var(--mono)', cursor: 'pointer' };
const delBtn    = { padding: '6px 12px', background: 'transparent', border: '1px solid #ff444433', borderRadius: 6, color: 'var(--red)', fontSize: 9, letterSpacing: '0.1em', fontFamily: 'var(--mono)', cursor: 'pointer' };
